...
// get all the rna sequences
const rnaSequences = async function (req, callback) {

    const apiUrl = url.bioinfo.base + url.bioinfo.rnaSequences;

    // get the user_id from the header data
    const data = await jwtToken.getDataFromToken(req.headers["authorization"]); // get the data from header token
...
async function dbDATA(request){
    ...
    if (user_id>0) {
        ...
        if (urs_id>0) {
            ...
            if (dataURS.rna_sequence === rna_sequence) {
                ...
                bCreatRNASequence = true;
            }
        } else { 
            bCreatRNASequence = true;
        }       

        if (bCreatRNASequence) {
            // create the users rna sequence data since it has not been created or it differs
            const bCreate = await bioDB.create_users_rna_sequences(user_id, rna_name, rna_sequence);
...
const saveBioInfoData = async function (req, callback) {
    ...
    try {
            ...
            if (urs_id>0){
                ...
                if (!await bioDB.create_urs_nussinov_predicted_structure(urs_id, dataObj.nussinov_structure)){
                    console.log('error nussinov');
                };

                // create the predicted structure for zuker
                if (!await bioDB.create_urs_zuker_predicted_structure(urs_id, dataObj.zuker_structure)){
                    console.log('error zuker');
                };

                // create the bio info
                if (!await bioDB.create_urs_sequences_bio_info(urs_id, dataObj.bio_info_details)){
                    console.log('error bio info');
                };

                // create the amino acid data
                if (!await bioDB.create_urs_sequences_amino_acids(urs_id, dataObj.amino_acids_data)){
